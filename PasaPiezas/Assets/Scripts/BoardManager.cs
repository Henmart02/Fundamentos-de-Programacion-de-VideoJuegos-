...
[HideInInspector] public bool waitingForPromotion = false;
    ...
    private bool lastWasEnPassant = false;
private Vector2Int lastEnPassantCapturedPosition;

    private Piece lastMovedPiece;
private Vector2Int lastFrom;
private Vector2Int lastTo;
private Piece lastCapturedPiece;
private bool hasLastMove = false;
public bool lastMoveWasWhite;
private bool lastMoveWasPromotion = false;
private PieceType lastOriginalPieceType;
private bool lastMoveWasPocketPlacement = false;
private PieceType lastPocketPieceType;
private bool lastPocketPieceIsWhite;
private Vector2Int lastPocketPosition;
private Piece lastPocketPlacedPiece;
...
return transform.position + new Vector3((x + offsetX) * tileSize, (y + offsetY) * tileSize, z);
...
public Vector3 BoardToWorldPosition(int x, int y, float z = -1f)
{
    ...
    void CreatePiece(PieceType type, bool isWhite, int x, int y)
    {
        ...
        pieceObj.transform.localScale = new Vector3(tileSize * 0.35f, tileSize * 0.35f, 1f);
        //pieceObj.transform.localScale = new Vector3(0.3f, 0.3f, 1f);
        //pieceObj.transform.localScale = new Vector3(0.2f, 0.2f, 1f);
/*
    ...
    public bool TryMovePiece(Piece piece, int newX, int newY)
{
    ...
    if (EndGameManager.Instance != null && EndGameManager.Instance.gameEnded)
{
    Debug.Log("La partida ya terminó.");
    return false;
}

    if (BughouseManager.Instance != null && BughouseManager.Instance.gameEnded)
{
    Debug.Log("La partida ya terminó.");
    return false;
}
...
lastFrom = new Vector2Int(piece.boardX, piece.boardY);
lastTo = new Vector2Int(newX, newY);
lastCapturedPiece = GetPieceAt(newX, newY);
hasLastMove = true;*/
lastMovedPiece = piece;
lastMoveWasWhite = piece.isWhite;
lastMoveWasPromotion = false;
lastOriginalPieceType = piece.pieceType;
lastMoveWasPocketPlacement = false;
lastFrom = new Vector2Int(piece.boardX, piece.boardY);
lastTo = new Vector2Int(newX, newY);
lastCapturedPiece = GetPieceAt(newX, newY);
hasLastMove = true;
...
lastEnPassantCapturedPosition = Vector2Int.zero;
...
{
    int capturedPawnY = piece.boardY;
    lastCapturedPiece = GetPieceAt(newX, capturedPawnY);
    lastEnPassantCapturedPosition = new Vector2Int(newX, capturedPawnY);
...
bool wasCapture = GetPieceAt(newX, newY) != null;
ExecuteMove(piece, newX, newY, true);
/*
...
bool enemyTurn = !piece.isWhite;

Debug.Log("Revisando jaque mate contra: " + (enemyTurn ? "Blancas" : "Negras"));

if (IsKingInCheck(enemyTurn))
{
    if (AudioManager.Instance != null)
        AudioManager.Instance.PlayCheckSound();
}

if (IsCheckmate(enemyTurn))
{
    Debug.Log("JAQUE MATE DETECTADO");

    if (AudioManager.Instance != null)
    {
        AudioManager.Instance.PlayCheckmateSound();
    }

    // PASA PIEZAS
    if (BughouseManager.Instance != null &&
        BughouseManager.Instance.GetPartnerBoard(this) != null)
    {
        BughouseManager.Instance.EndGameByCheckmate(this, piece.isWhite);
    }
    ...
    else if (EndGameManager.Instance != null)
    {
        EndGameManager.Instance.EndGame(
            (piece.isWhite ? "Ganan las blancas" : "Ganan las negras")
            + "\nJAQUE MATE"
        );
    }
...
if (isEnPassantMove && enPassantPawn != null)
{
    ...
    enPassantPawn.gameObject.SetActive(false);
}
...
lastOriginalPieceType = PieceType.Pawn;
        promotionPawn = piece;
...
public bool TryDropPocketPiece(bool isWhite, PieceType pieceType, int x, int y)
{
    ...
    if (AudioManager.Instance != null)
{
    AudioManager.Instance.PlayPocketPlaceSound();
}
...
// Guardar esta colocación como última jugada
lastMoveWasPocketPlacement = true;
lastPocketPieceType = pieceType;
lastPocketPieceIsWhite = isWhite;
lastPocketPosition = new Vector2Int(x, y);
lastPocketPlacedPiece = piece;

lastMovedPiece = null;
hasLastMove = true;
lastMoveWasWhite = isWhite;
...
public void SelectPiece(Piece piece)
{
    ...
    foreach (Vector2Int move in legalMoves)
    {
        if (tileGrid[move.x, move.y] != null)
            tileGrid[move.x, move.y].HighlightGreen();
    }
}
...
public void UndoLastMove()
{
    if (!hasLastMove)
    {
        Debug.Log("No hay jugada para deshacer.");
        return;
    }

    // Si la última jugada fue colocar pieza desde pocket
    if (lastMoveWasPocketPlacement)
    {
        if (lastPocketPlacedPiece != null)
        {
            pieceGrid[lastPocketPosition.x, lastPocketPosition.y] = null;
            Destroy(lastPocketPlacedPiece.gameObject);
        }

        if (BughouseManager.Instance != null)
        {
            BughouseManager.Instance.AddPieceBackToPocket(
                this,
                lastPocketPieceIsWhite,
                lastPocketPieceType
            );
        }
        ...
        hasLastMove = false;
        lastMoveWasPocketPlacement = false;
        lastPocketPlacedPiece = null;

        UpdateCheckHighlight();

        Debug.Log("Se deshizo colocación desde pocket.");
        return;
    }

    if (lastMovedPiece == null)
    {
        Debug.Log("No hay pieza movida para deshacer.");
        return;
    }

    // VALIDACIÓN ANTES DE DESHACER
    if (lastCapturedPiece != null &&
    BughouseManager.Instance != null &&
    BughouseManager.Instance.GetPartnerBoard(this) != null)
{
    bool partnerStillHasPiece = BughouseManager.Instance.PartnerStillHasCapturedPiece(
        this,
        lastMovedPiece.isWhite,
        lastCapturedPiece.pieceType
    );

    if (!partnerStillHasPiece)
    {
        Debug.Log("No se puede deshacer: el compañero ya usó la pieza capturada.");
        return;
    }
}

    // Desde aquí sí se deshace
    pieceGrid[lastTo.x, lastTo.y] = null;
    pieceGrid[lastFrom.x, lastFrom.y] = lastMovedPiece;

    lastMovedPiece.SetBoardPosition(lastFrom.x, lastFrom.y);
    lastMovedPiece.transform.SetParent(null);
    lastMovedPiece.transform.position = BoardToWorldPosition(lastFrom.x, lastFrom.y, -1);

    if (lastMoveWasPromotion)
{
    lastMovedPiece.pieceType = PieceType.Pawn;

    SpriteRenderer sr = lastMovedPiece.GetComponent<SpriteRenderer>();
    if (sr != null)
        sr.sprite = GetPromotionSprite(PieceType.Pawn, lastMovedPiece.isWhite);
    ...
    if (lastCapturedPiece != null)
    {
        lastCapturedPiece.gameObject.SetActive(true);

        if (BughouseManager.Instance != null &&
    BughouseManager.Instance.GetPartnerBoard(this) != null)
{
    BughouseManager.Instance.RemoveCapturedPieceFromPartner(
        this,
        lastMovedPiece.isWhite,
        lastCapturedPiece.pieceType
    );
        ...
        Vector2Int restorePos = lastWasEnPassant ? lastEnPassantCapturedPosition : lastTo;

        pieceGrid[restorePos.x, restorePos.y] = lastCapturedPiece;

        lastCapturedPiece.SetBoardPosition(restorePos.x, restorePos.y);
        lastCapturedPiece.transform.SetParent(null);
        lastCapturedPiece.transform.position = BoardToWorldPosition(restorePos.x, restorePos.y, -1);
    }
    ...
    if (lastWasEnPassant)
    {
        enPassantTarget = lastTo;
        enPassantPawn = lastCapturedPiece;
    }
    else
    {
        enPassantTarget = null;
